...
| Детектор повторов | — | один и тот же вызов трижды подряд обрывает цикл |
...
| `OLLAMA_URL` / `OLLAMA_MODEL` / `OLLAMA_NUM_THREAD` | `http://localhost:11434` / `qwen3:1.7b` / — | Настройки `ollama` |
