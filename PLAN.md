...
graph LR
    ...
    subgraph A[Процесс агента]
        ...
        HR <--> PL[Плагин инференса<br/>ollama / openai_compat / echo]
...
Слои заменяются независимо друг от друга:

- **Транспорт** — как апдейты попадают в бота. `polling` ходит за ними сам, `webhook` принимает их POST-запросом. Выбирается `TELEGRAM_TRANSPORT`. На приём работает только он: ответы бот шлёт напрямую через `TelegramClient`, мимо транспорта.
- **Обработчик** — бизнес-логика и контекст чата. Не знает ни транспорта, ни провайдера.
- **Агент** — отдельный процесс, внутри которого живут харнесс, плагин инференса и инструменты. Провайдер выбирается `LLM_PROVIDER`.
- **Инструменты и скиллы** — `exec` выполняет команды, `skill` читает инструкции из `skills/*.md`. Новый инструмент — файл в `bot/tools/`, новый скилл — файл с текстом, кода не требует.
...
| Модель придумывает результат вызова | Прямой запрет в системном промпте и в напоминании к скиллу; к тексту ошибки инструмента приклеен запрет подставлять значения; ответ после единственного упавшего вызова не принимается — модель возвращается к вызову |
...
| Модель молчит на финальном шаге | Пустой ответ — отдельный тип ошибки, а не отказ провайдера: харнесс переспрашивает, собранные данные не выбрасываются. Причину снимают `OLLAMA_THINK=false` и `OLLAMA_NUM_CTX=8192` |
